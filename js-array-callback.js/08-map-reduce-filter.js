...
const numbersg = [ 4, 10, 3, 0, -2, -5, 1 ];
// Count evens = 4

const evenNumbs = numbers.reduce((acc , curr) => {
...
const elementsWithApple = fruits.reduce((acc , curr) => {
    if(curr.toLowerCase().includes('apple')) return  acc+1;
return acc;
} , 0);

console.log(elementsWithApple);

const elementsWithI = fruits.reduce((acc, curr) =>{
    if(curr.toLowerCase().indexOf('i') )});
